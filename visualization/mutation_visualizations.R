...
# of MAF files, primarily using maftools
...
# Link to maftools package: https://bioconductor.org/packages/release/bioc/html/maftools.html
...
summarize_maf <- function(maf) {
  ...
  plotmafSummary(maf = maf, rmOutlier = T, addStat = 'median', 
                 dashboard = T, titvRaw = F)
  return(gene_summary)
...
#' is specified as T. This only includes primary solid tumor samples.
#' @param maf a MAF file that has been read in by maftools
...
get_avg_muts_per_gene <- function(mut_count_matrix, maf_file, top_gene) {
  index <- which(rownames(mut_count_matrix) == top_gene)
  gene_avg_muts <- mean(as.numeric(mut_count_matrix[index,]))
  lollipopPlot(maf = maf_file, gene = top_gene, showMutationRate = T)
...
get_num_mutations_per_patient <- function(mut_count_matrix) {
  ...
  # hist(total_mut_count_df$Sample.1, xlab = "Total Number of Mutations", 
  ...
  #boxplot(total_mut_count_df$Sample.1, ylab = "Total Number of Mutations")
    #identify(rep(1, length(total_mut_count_df$Sample.1)), 
    #total_mut_count_df$Sample.1, 
...
visualize_mutation_distrib <- function(total_mutation_count_matrix, 
  ...
  mut_count_matrices <- lapply(cancer_types, function(ct) {
    # Subset the mapping to just this cancer type & get all the patients of 
    # this type
    ...
    count_matrix_sub <- total_mutation_count_matrix[
      grepl(paste(patients, collapse = "|"), 
            rownames(total_mutation_count_matrix)), , drop = F]
...
make_patient_id_cancer_type_map <- function(patient_ids, clinical_df) {
  ...
  rows <- lapply(patient_ids, function(id) {
    ...
    project_id <- unlist(strsplit(unique(clinical_df[
      grepl(id, clinical_df$case_submitter_id), 'project_id']), "-", 
      fixed = T))[2]
...
# Import needed information, see process_mutation_data.R
clinical_df <- read.csv(paste0(PATH, "Mutation/clinical_data.csv"), header = T)
total_mutation_count_matrix <- read.csv(paste0(
  PATH, "Mutation/total_mut_count_matrix_per_patient.csv"))
patient_ids <- unique(unlist(lapply(total_mutation_count_matrix$X, function(x) 
...
total_mut_count_tab <- visualize_mutation_distrib(total_mutation_count_matrix, 
...
#' @param clinical_df an aggregated clinical DF across all cancer types in 
#' the TCGA
get_per_cancer_sub_mafs <- function(maf, clinical_df) {
  
  # Get a mutation count matrix for every cancer type; split the maf file 
  # into sub-mafs based on patient cancer types
  patient_ids <- unique(unlist(lapply(maf$Tumor_Sample_Barcode, function(x) 
    unlist(strsplit(x, "-", fixed = T))[3])))
  patient_id_cancer_type_map <- make_patient_id_cancer_type_map(patient_ids, 
                                                                clinical_df)
  ...
  lapply(cancer_types, function(ct) {
    # Subset the mapping to just this cancer type & get all the patients 
    # of this type
    patients <- patient_id_cancer_type_map[
      patient_id_cancer_type_map$Project.ID == ct, 'Patient.ID'] 
    # Use these patients to subset the MAF file
    maf_sub <- maf[grepl(patients, maf$Tumor.Sample.Barcode),]      
    
    # Write this MAF to a file
    write.table(maf_sub, paste0(PATH, paste0("Mutation/Sub-MAF Files/maf_file_", 
                                            paste0(ct, ".maf"))))
  }) 
}
...
#' VISUALIZE THE NUMBER OF PATIENTS WITH MUTATIONS IN EACH PROTEIN
#' Given a MAF file (filtered as desired), a subsetted domains DF and a label, 
#' creates barplots/ boxplots of the frequency of binding among
...
visualize_mutation_freqs <- function(all_gene_id_conv, domain_df, 
                                     label, maf_df) {
  
  # Get the protein IDs from the domain DF
  if (label == "I-Protein") {
    protein_ids <- unique(unlist(lapply(domain_df$Query, function(x) 
  ...
  if (label == "I-Protein") {
    ...
    freq_df$Num.Patients <- unlist(lapply(protein_ids, function(x) 
      length(unique(maf_df[grepl(x, maf_df$SWISSPROT), 'Tumor_Sample_Barcode']))))
  } else if (label == "I-Domain") {
    freq_df$Num.Patients <- unlist(lapply(protein_ids, function(x) 
      length(unique(domain_df[grepl(x, domain_df$Swissprot),'Patient']))))
  } else if (label == "I-Binding Position") {
    freq_df$Num.Patients <- unlist(lapply(protein_ids, function(x) 
      length(unique(domain_df[domain_df$Protein.ID == x,'Patient']))))
  } else {
    print(paste("Invalid label:", label))
  }
  
  freq_df <- freq_df[order(freq_df$Num.Patients, decreasing = T),]
  
  # Create barplots
  ylab_label <- strsplit(strsplit(label, " ", fixed = T)[1], "-", fixed = T)[2]
  ...
  print(paste("Percent above 1% frequency:", 
              (length(col_freq[col_freq >1])/length(col_freq))*100))
  print(paste("Number of proteins with >1% frequency:", 
              length(col_freq[col_freq > 1])))
  print(paste("Number of proteins mutated in patients more than 3 times:", 
              length(freq_df$Num.Patients[freq_df$Num.Patients >3])))
  print(paste("Number of proteins mutated in patients more than 6 times:", 
              length(freq_df$Num.Patients[freq_df$Num.Patients >6])))
...
create_barplot <- function(freq_df, label, ylab_label) {
  barplot(freq_df$Num.Patients, main = 
            paste(label, " Number of Patients with Mutations in Each Protein", 
                  sep = ":"), 
          xlab = "Protein", 
          ylab = paste("# of Patient Tumor Samp. with >1 Missense Mut. 
                       in Ligand-Binding ", ylab_label, sep = ""), col = "blue") 
}
...
create_boxplot <- function(freq_df, label) {
  bp = boxplot(freq_df$Freq, main = 
                 paste(label, " Frequency of Mutation across patients", sep = ":"), 
               ylab = "Frequency",
...
create_histogram <- function(freq_df, label) {
  hist(freq_df$Num.Patients, main = 
         paste(label, " Frequency of Mutations across Patient Population", 
               sep = ":"), 
       xlab = "Number of Patients with Mutated Version of Protein", 
       ylab = "Number of Genes", 
       col = brewer.pal(n = 11, name = "RdBu"), 
       xlim = c(2, max(freq_df$Num.Patients)))
}
...
create_cumulative_histogram <- function(freq_df, label) {
  ...
  for (i in 1:max(freq_df$Num.Patients)) {
    cum_vals <- c(cum_vals, 
                  length(freq_df$Num.Patients[freq_df$Num.Patients >= i]))
  }
  print(cum_vals)
  
  barplot(height = cum_vals[4:20], main = 
            paste(label, " Frequency of Mutations across Patient Population", 
                  sep = ":"), 
          xlab = "Minimum # of Patients with Mutated Version of Protein", 
          ylab = "Number of Proteins", col = 'cornflowerblue',
          border = 'black', space = 0, xpd = F, names.arg = c(4:20), 
          cex.lab = 1.5, cex.axis = 1.5, cex.main = 1.5, cex.sub = 1.5)
}


# Call function
frequency_df_iprotein <- visualize_mutation_freqs(all_gene_id_conv, 
                                                  domains_iprotein_sub, 
                                                  "I-Protein", 
                                                  maf_subset_df)
frequency_df_idomain <- visualize_mutation_freqs(all_gene_id_conv, 
                                                 domains_idomain_sub, 
                                                 "I-Domain")
frequency_df_ibindingpos <- visualize_mutation_freqs(all_gene_id_conv, 
                                                     domains_ibindingpos_sub, 
                                                     "I-Binding Position")
...
#' categories
get_num_domains_per_ligand <- function(domains, interacdome_df, canbind_df_sub, 
                                       ligand_groups) {
  
  # Get the domains from the domain DF
  doms <- domains$Accession
  doms_stripped <- unique(as.character(
    regmatches(doms, gregexpr("[[:digit:]]+", doms)))) # strip the PF from the name
  print(doms_stripped)
  
  # Create a list to hold counts of the results
  domain_types_list <- list("DNA" = 0, "RNA" = 0, "SM" = 0, "ION" = 0, 
                            "PEPTIDE" = 0, "NUCACID" = 0)
  ...
  interac_ids_stripped <- unlist(lapply(interacdome_df$pfam_id, function(x) 
    unlist(strsplit(unlist(strsplit(x, "_", fixed = T))[1], "F", fixed = T))[2]))
  ...
  for (i in 1:length(doms_stripped)) {
    domain_types_list <- get_ligand_types(i, interacdome_df, canbind_df_sub, 
                                          ligand_groups)
  }
  
  # Make this list into a data frame for visualization, ordered by count
  domain_types_table <- data.frame(matrix(unlist(domain_types_list), 
                                          nrow = length(domain_types_list),
  ...
  barplot(domain_types_table$Count, names.arg = domain_types_table$Category, 
          xlab = "Ligand Type", ylab = "Number of Binding Domains", 
          col=brewer.pal(n = 6, name = "RdBu"),
          main = "Number of domains that bind each ligand type, in top proteins", 
          ylim = c(0,6000))
...
get_ligand_types <- function(i, interacdome_df, canbind_df_sub, 
                             domain_types_list) {
  # Check InteracDome
  ligand_types_interac <- interacdome_df[
    interacdome_df$stripped_pfam_ids == doms_stripped[i], 'ligand_type']
  
  # Check CanBind
  prot_id <- unlist(strsplit(domains$Query[i], "|", fixed = T))[2]
  ligand_types_canbind <- canbind_df_sub[canbind_df_sub$Swissprot == prot_id, 
                                         'LigandType']
  ...
  if (!length(ligand_types) == 0) {
    for (j in 1:length(ligand_types)) {
      domain_types_list <-check_lig_type(ligand_types[j], domain_types_list, 
                                         ligand_groups)
    }
  }
...
############################################################
#' Helper function that uses the ligand groups document to sort encountered 
#' ligands into buckets. Takes a ligand type, a list of domain types to fill in, 
#' and the ligand groups document.Returns the updated domain types list.
...
check_lig_type <- function(ligand_type, domain_types_list, ligand_groups) {
  ...
  } else if (ligand_type %fin% ligand_groups$original_ligand_mmCIF_identifier) { 
    broad_cat <- ligand_groups[ligand_groups$original_ligand_mmCIF_identifier == 
                                 ligand_type, 'group_name']
...
# Call function
get_num_domains_per_ligand(domains_iprotein_sub, interacdome_df, 
                           canbind_df_sub, ligand_groups)
get_num_domains_per_ligand(domains_idomain_sub, interacdome_df, 
                           canbind_df_sub, ligand_groups)
get_num_domains_per_ligand(domains, interacdome_conf, 
                           canbind_df_sub, ligand_groups)  
...
get_num_proteins_that_bind_each_lig_type <- function(domains, interacdome_dw_sub, 
                                                     canbind_df_sub, 
                                                     ligand_groups, label, 
                                                     ibinding_ids) {
  
  # If I-Protein, we can get the Swissprot IDs of all the proteins we're 
  # interested in from the domains DF; otherwise import them as a parameter
  if(label == "I-Protein") {
    proteins <- unique(unlist(lapply(domains$Query, function(x) 
      unlist(strsplit(x, "|", fixed = T))[2])))
  } else {proteins <- ibinding_ids}
  
  # Create columns of stripped IDs for InteracDome for easier comparison
  interac_ids_stripped <- unlist(lapply(interacdome_dw_sub$domain_name, function(x) 
    unlist(strsplit(unlist(strsplit(x, "_", fixed = T))[1], "F", fixed = T))[2]))
  ...
  for (i in 1:length(proteins)) {
    ...
    doms_stripped <- unique(as.character(regmatches(doms, gregexpr("[[:digit:]]+", 
                                                                   doms))))
    
    # For all domains, check the ligand type they bind
    for (j in 1:length(doms_stripped)) {
      domain_types_list <- get_ligand_types(j, interacdome_dw_sub, 
                                            canbind_df_sub, domain_types_list)
    }
  ...
  counts <- list("DNA" = num_dna_binders, "RNA" = num_rna_binders, 
                 "PEPTIDE" = num_pep_binders, "SM" = num_sm_binders, 
                 "ION" = num_ion_binders)
  ...
  barplot(counts_table$Count, names.arg = counts_table$Category, 
          xlab = "Ligand Type", ylab = "Number of Proteins", 
          col=brewer.pal(n = 6, name = "RdBu"),
          main = "Number of proteins that bind each ligand type", 
          ylim = c(0,length(proteins)))
  ...
  proportions <- list("DNA" = prop_dna_binders, "RNA" = prop_rna_binders, 
                      "PEPTIDE" = prop_pep_binders, "SM" = prop_sm_binders, 
                      "ION" = prop_ion_binders)
  
  # Make this list into a data frame for visualization, ordered by count
  proportions_table <- data.frame(matrix(unlist(proportions), 
                                         nrow = length(proportions), byrow = T), 
                                  stringsAsFactors = F)
  ...
  barplot(proportions_table$Proportion, names.arg = proportions_table$Category, 
          xlab = "Ligand Type", ylab = "Proportion of Proteins", 
          col=brewer.pal(n = 6, name = "RdBu"),
          main = "Proportion of proteins that bind each ligand type", 
          ylim = c(0,1))
}
...
# Call function
get_num_proteins_that_bind_each_lig_type(domains_iprotein_sub, 
                                         interacdome_domainweights, 
                                         canbind_df_sub, ligand_groups, 
                                         "I-Protein")
get_num_proteins_that_bind_each_lig_type(domains_idomain_sub, 
                                         interacdome_domainweights, 
                                         canbind_df_sub, ligand_groups, 
                                         "I-Domain", swissprot_ids_idomain)
get_num_proteins_that_bind_each_lig_type(domains_idomain_sub, 
                                         interacdome_domainweights, 
                                         canbind_df_sub, ligand_groups, 
                                         "I-Binding Position", 
...
get_num_proteins_that_bind_each_lig_type(domains, interacdome_conf, 
                                         canbind_df_sub, ligand_groups)  
...
get_most_mutated_drivers_per_ct <- function(cancer_types, clinical_df, 
                                            regprot_mut_df, driver_gene_df, N, 
                                            all_genes_id_conv) {
  
  top_mutated_genes_per_ct <- lapply(cancer_types, function(ct) {
    # Get the patients that are of this cancer type
    ct_patients <- unlist(clinical_df[grepl(ct, clinical_df$project_id), 
                                      'case_submitter_id'])
    ct_patients <- unlist(lapply(ct_patients, function(x) 
      unlist(strsplit(x, "-", fixed = T))[3]))
    ...
    for (i in 1:nrow(regprot_mut_df)) {
      ...
      new_pats <- c()
      overlap <- F
      # Check each patient in this cancer type; if they are listed here, 
      # keep them, and discard all other patients
      for(j in ct_patients) {
        if(T %fin% grepl(j, pats)) {
          overlap <- T
      ...
      if(overlap == T) {
        regprot_mut_df_sub_patients <- rbind(
          regprot_mut_df_sub_patients, data.frame(
            "Swissprot" = regprot_mut_df$Swissprot[i], 
    ...
    count_tab <- data.frame(protein = unique(regprot_mut_df_sub_patients$Swissprot),
                            num_patients_with_mutation = 
                              rep(NA, length(unique(
                                regprot_mut_df_sub_patients$Swissprot))))
    for (prot in unique(regprot_mut_df_sub_patients$Swissprot)) {
      pats <- regprot_mut_df_sub_patients[
        grepl(prot, regprot_mut_df_sub_patients$Swissprot),'Patient']
      unique_pats <- unique(unlist(strsplit(pats, ";", fixed = T)))
      count_tab[count_tab$protein == prot, 
                'num_patients_with_mutation'] <- length(unique_pats)
    }
    ...
    count_tab_list <- lapply(1:nrow(count_tab), function(i) {
      ensg_ids <- unlist(strsplit(count_tab$ensg[i], ";", fixed = T))
      if(any(ensg_ids %fin% driver_gene_df$ensembl_gene_id)) {
        return(count_tab[i,])
      }
    })
    ...
    ordered_count_tab <- count_tab_drivers[
      order(-count_tab_drivers$num_patients_with_mutation),]
...
cancer_types_of_interest <- c("BRCA", "LGG", "THCA", "PRAD", "HNSC", "LIHC", 
                              "LUAD", "BLCA", "UCEC", "STAD", "KIRP", "KIRC", 
                              "CESC", "LUSC", "COAD", "SARC", "PCPG", "ESCA", 
                              "PAAD", "TGCT")

regprot_mut_df <- read.csv(paste0(PATH, "Mutation/iprotein_results.csv"), 
                           header = T, check.names = F)

driver_gene_df <- read.table(paste0(PATH, "GRCh38_driver_gene_list.tsv"),
                             header = T, check.names = F)

# Call function
top_drivers_per_ct <- get_most_mutated_drivers_per_ct(cancer_types_of_interest, 
                                                      clinical_df, regprot_mut_df,
                                                      driver_gene_df, 10, 
                                                      all_genes_id_conv)

# Optionally convert to DF and add the gene name
top_drivers_per_ct_df <- as.data.frame(top_drivers_per_ct)
protein_sub <- top_drivers_per_ct_df[,grepl(".protein", 
                                            colnames(top_drivers_per_ct_df))]
new_cols <- lapply(1:ncol(protein_sub), function(i) {
  col <- protein_sub[,i]
  names <- unlist(lapply(col, function(x) {
    paste(unique(unlist(all_genes_id_conv[all_genes_id_conv$uniprot_gn_id == x,
                                          'external_gene_name'])), 
          collapse = ";")}))
  return(names)
...
colnames(new_cols_df) <- unlist(lapply(cancer_types_of_interest, function(ct) 
  paste(ct, "geneName", sep = ".")))
...
top_drivers_per_ct_df2$gene_name <- unlist(lapply(
  top_drivers_per_ct_df2$protein, function(x) {
  paste(unique(unlist(all_genes_id_conv[all_genes_id_conv$uniprot_gn_id == x, 
                                        'external_gene_name'])), 
        collapse = ";")}))

ggplot(top_drivers_per_ct_df2, aes(
  y = reorder_within(gene_name, by = num_patients_with_mutation, 
                     within = cancer_type, sep = "_"), 
  x = num_patients_with_mutation)) + geom_bar(stat = "identity") + 
  labs(x = "Driver Gene", y = "Num. Patients with Mutation") + 
  facet_wrap(~cancer_type, scales = "free")


############################################################
#' DETERMINE CORRELATION BETWEEN MUTATION STATUS OF KEY GENES AND 
#' CANCER SUBTYPE IN BRCA
#' Creates a heat map that displays the count of patients that have a given 
#' subtype and a corresponding mutation in a particular gene of interest
#' @param subtype_file contains the subtypes for the given cancer along with the 
#' corresponding patient IDs
#' @param mutation_regprot_df a data frame with the mutation status of patients 
#' in the given cancer type for the genes of interest
#' @param genes_of_interest a vector of genes of interest, swissprot IDs
#' @param totOrFrac either "total" or "fraction" to denote whether we want the 
#' totalcount of mutated patients for each gene, or the fraction of patients 
#' that have a mutation in this gene
visualize_mutation_corr_to_subtype <- function(subtype_file, mutation_regprot_df, 
  ...
  count_mat <- data.frame(matrix(nrow = length(genes_of_interest), 
  ...
  if(totOrFrac == "fraction") {
    total_counts <- list("Basal" = 0, "Her2" = 0, "LumB" = 0, 
                         "LumA" = 0,"Normal" = 0)
  }
  ...
  for (i in 1:length(genes_of_interest)) {
    ...
    for (j in 1:length(patients)) {
      patient <- patients[j]
      subtype <- as.character(unlist(subtype_file[subtype_file$patient == patient, 
                                                  'BRCA_Subtype_PAM50']))

      if (!(subtype == "NA")) {
        pat_id <- unlist(strsplit(patient, "-", fixed = T))[3]

        # If this patient has a mutation in this gene, add to count
        mutation_regprot_df_sub <- mutation_regprot_df[
          mutation_regprot_df$Swissprot == gene,]
        if(any(grepl(pat_id, mutation_regprot_df_sub$Patient))) {
          count_mat[i, colnames(count_mat) == subtype] <- count_mat[
            i, colnames(count_mat) == subtype] + 1
        }
        if(totOrFrac == "fraction") {
          total_counts[names(total_counts) == subtype] <- as.integer(
            total_counts[names(total_counts) == subtype]) + 1
        }
      }
    }
  }
  ...
  p <- ggplot(count_mat_melt, aes(x = Subtype, y = Gene.name, 
                                  fill = Mutation.Count)) + 
    geom_tile() + 
    geom_text(aes(label = Mutation.Count), color = "black", size = 6) + 
    theme(axis.title.y=element_blank(), axis.text = element_text(size = 14), 
          axis.title.x = element_text(size = 16)) + 
    scale_fill_gradient(low = "dark orange", high = "yellow", guide = "colorbar")
  print(p)
...
# Import a sample subtype file using TCGA Biolinks
brca_subtype <- TCGAquery_subtype(tumor = "BRCA")

# Import regulatory protein mutation data frame
mutation_regprot_df <- fread(paste0(PATH, "Mutation/iprotein_results.csv"),
                             header = T)   # I-Protein, all ligands
...
genes_of_interest_brca_swissprot <- c("P42336", "P04637", "P31749", "Q8NEZ4", 
                                      "Q13233", "P60484", "Q14517", "P45985", 
                                      "P04626", "Q7Z6Z7", "P55317") 

count_matrix <- visualize_mutation_corr_to_subtype(brca_subtype, 
                                                   mutation_regprot_df, 
                                                   genes_of_interest_brca_swissprot,
                                                   "total")
count_matrix_frac <- visualize_mutation_corr_to_subtype(brca_subtype, 
                                                        mutation_regprot_df, 
                                                        genes_of_interest_brca_swissprot,
                                                        "fraction")
...
#' @param PATH a PATH to where we should write the new file
#' @param subtypes_label a character label of what subtype(s) we're looking at in
#' order to appropriately name the new file
get_subtype_patient_list <- function(subtype_file, subtypes_of_interest, PATH,
                                     subtypes_label) {
  output_patient_ids <- c()
  
  for (subtype in subtypes_of_interest) {
    new_ids <- subtype_file[subtype_file$BRCA_Subtype_PAM50 == subtype, 
                            "patient"]  #TODO: make this generalizable beyond BRCA
    new_ids_short <- unlist(lapply(new_ids$patient, function(id) 
      unlist(strsplit(id, "-", fixed = T))[3]))
    output_patient_ids <- c(output_patient_ids, new_ids_short)
  }
  
  fwrite(as.data.table(output_patient_ids), 
         paste0(PATH, paste0(subtypes_label, "_patient_ids.txt")))
}
...
# Call function
get_subtype_patient_list(brca_subtype, c("LumA", "LumB"), PATH, "Luminal.A.B")
get_subtype_patient_list(brca_subtype, "LumA", PATH, "Luminal.A")


############################################################
#' DETERMINE THE NUMBER OF FUNCTIONAL COPIES OF A GIVEN GOI FOR EACH PATIENT SAMPLE
#' Given a gene-of-interest, a MAF file, and a CNA file (with absolute CNs), 
#' determine for each patient sample the number of functional copies they have 
#' of the given GOI
#' @param maf_file a TCGA MAF file, subsetted to only missense and 
#' nonsense mutations
...
#' @param ts_or_onco a character string denoting whether the goi is a 
#' "tumor_suppressor" or an "oncogene"
get_num_functional_copies <- function(maf_file, cna_file, patient_ids, goi, 
                                      ts_or_onco) {
  ...
  maf_sub$Patient_ID <- unlist(lapply(maf_sub$Tumor_Sample_Barcode, function(x)
    unlist(strsplit(x, "-", fixed = T))[3]))
  maf_sub <- maf_sub[maf_sub$Patient_ID %fin% patient_ids,]
  
  cna_pats <- unlist(lapply(colnames(cna_sub), function(x)
    unlist(strsplit(x, "-", fixed = T))[1]))
  cna_sub <- cna_sub[,which(cna_pats %fin% patient_ids)]
  ...
  output_df$Num.Missense.Mut.Copies <- get_num_mut_copies(output_df, maf_sub, 
                                                          "Missense_Mutation")
  output_df$Num.Nonsense.Mut.Copies <- get_num_mut_copies(output_df, maf_sub, 
                                                          "Nonsense_Mutation")
  output_df$Num.Mut.Copies <- get_num_mut_copies(output_df, maf_sub, 
                                                 c("Missense_Mutation", 
                                                   "Nonsense_Mutation"))
  ...
  output_df$Num.Functional.Copies <- unlist(lapply(1:nrow(output_df), function(i) {
    ...
    # If we have one copy deleted, then we know we're down at least one 
    # functional copy; need to check and see if the other has a mutation, if a TS
    ...
    # If we have at least 2 non-deleted copies, we just have to check for 
    # mutation status 
    } else {
      if (ts_or_onco == "tumor_suppressor") {
        # If we have at least 1 mutated allele, subtract this from the total CN
        if(output_df[i, 'Num.Mut.Copies'] >= 1) {
          return(as.integer(output_df[i, 'Abs.CN']) - as.integer(
            output_df[i, 'Num.Mut.Copies']))
        }
        # If we have no mutated alleles, and no deletions, we have 2 (or more) 
        # functional copies
        else {as.integer(output_df[i, 'Abs.CN'])}
      } else {
        # If this is an oncogene, we have to check whether the mutation is 
        # nonsense (If missense, likely GOF, and if nonsense, LOF)
        if(output_df[i, 'Num.Nonsense.Mut.Copies'] >= 1) {
          return(as.integer(output_df[i, 'Abs.CN']) - 
                   as.integer(output_df[i, 'Num.Nonsense.Mut.Copies']))
        }
        else {as.integer(output_df[i, 'Abs.CN'])}
      }
    }
...
get_num_mut_copies <- function(output_df, maf_sub, classification) {
  # Iterate through all the samples and, for each, get the number of mutant 
  # copies of a given classification
  num_mut_copies <- unlist(lapply(1:nrow(output_df), function(i) {
    ...
    if(length(classification) == 1) {
      ...
      maf_samp <- maf_sub[(grepl(samp, maf_sub$Tumor_Sample_Barcode)) & 
                            (maf_sub$Variant_Classification %fin% classification),]
    }
    
    num_mutant_copies <- 0
    print(nrow(maf_samp))
    
    # If there's no remaining rows (aka mutations), we have no mutated 
    # copies -- return 0
    ...
    # Otherwise, look at the individual alleles
      ...
      # If there are multiple rows of mutations, iterate through them to see 
      # which allele they are specific to
...
# Call function
output_df <- get_num_functional_copies(maf_df, cna_df, patient_ids, goi, 
                                       ts_or_onco)

# Visualize the distribution of functional copies
hist(output_df$Num.Functional.Copies, 
     breaks = seq(min(output_df$Num.Functional.Copies)-0.5, 
                  max(output_df$Num.Functional.Copies)+0.5, by =1),
     xlab = "Num. Functional Copies", 
     main = "TP53 Histogram of Num. Functional Copies (BRCA)")
...
hist(new_vals, breaks = seq(min(new_vals), max(new_vals)+0.5, by =1),
     xlab = "Log2(Num. Functional Copies + 1)", 
     main = "TP53 Histogram of Log2(Num. Functional Copies + 1) (BRCA)")

# Create a Venn diagram of patients with both a TP53 deletion and mutation
ggVennDiagram(list("P53 Deletion" = output_df[output_df$Abs.CN %fin% c(0,1), 
                                              'sample.id'], 
                   "P53 Mutation" = output_df[output_df$Num.Mut.Copies >= 1, 
                                              'sample.id']), 
              label_alpha = 0, 
              category.names = c("TP53 Deletion", "TP53 Mutation"), 
              set_color = "black", set_size = 10, label_size = 8, edge_size = 0) +
  scale_fill_gradient(low="cornsilk1", high = "cadetblue3")

# How many have no functional copies?
barplot(unlist(list("Total P53 KO" = length(output_df[
  output_df$Num.Functional.Copies == 0, 'sample.id']), 
  "Partial P53 KO (1 Cp)" = length(output_df[output_df$Num.Functional.Copies == 1, 
                                             'sample.id']),
  ">=2 Functional P53 Cp" = length(output_df[output_df$Num.Functional.Copies >= 2, 
                                             'sample.id']))))