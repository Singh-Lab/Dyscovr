############################################################
# Code to Create Figure 1 Visualizations
...
############################################################
### PART B-C: BARPLOT OF ENRICHMENT IN CGC AND STRING TARGETS
############################################################  
...
### STRING ###
# https://string-db.org/
# Generate set of string neighbors from STRINGdb
...
kras = string_db$mp( "kras" )
idh1 = string_db$mp( "idh1" )

tp53_string_neighbors <- string_db$get_neighbors(tp53)
pik3ca_string_neighbors <- string_db$get_neighbors(pik3ca)
kras_string_neighbors <- string_db$get_neighbors(kras)
idh1_string_neighbors <- string_db$get_neighbors(idh1)

# Convert these from ENSP to ENSG
tp53_neighbors_ensg <- ensembldb::select(EnsDb.Hsapiens.v86, 
                                         keys=unlist(lapply(tp53_string_neighbors, function(x) 
...
pik3ca_neighbors_ensg <- ensembldb::select(EnsDb.Hsapiens.v86, 
                                           keys=unlist(lapply(pik3ca_string_neighbors, function(x) 
                                             unlist(strsplit(x, ".", fixed = TRUE))[2])), 
                                           keytype = "PROTEINID", columns = c("SYMBOL","PROTEINID","GENEID"))
kras_neighbors_ensg <- ensembldb::select(EnsDb.Hsapiens.v86, 
                                         keys=unlist(lapply(kras_string_neighbors, function(x) 
                                           unlist(strsplit(x, ".", fixed = TRUE))[2])), 
                                         keytype = "PROTEINID", columns = c("SYMBOL","PROTEINID","GENEID"))
idh1_neighbors_ensg <- ensembldb::select(EnsDb.Hsapiens.v86, 
                                           keys=unlist(lapply(idh1_string_neighbors, function(x) 
                                             unlist(strsplit(x, ".", fixed = TRUE))[2])), 
                                           keytype = "PROTEINID", columns = c("SYMBOL","PROTEINID","GENEID"))

# Write to file
write.table(tp53_neighbors_ensg, paste0(PATH, "Validation_Files/tp53_stringdb_neighbors.txt"))
write.table(pik3ca_neighbors_ensg, paste0(PATH, "Validation_Files/pik3ca_stringdb_neighbors.txt"))
write.table(kras_neighbors_ensg, paste0(PATH, "Validation_Files/kras_stringdb_neighbors.txt"))
write.table(idh1_neighbors_ensg, paste0(PATH, "Validation_Files/idh1_stringdb_neighbors.txt"))

# Read back from file
tp53_string_neighbors <- read.table(paste0(PATH, "Validation_Files/tp53_stringdb_neighbors.txt"), 
                                   header = T, check.names = F)[,'SYMBOL']
pik3ca_string_neighbors <- read.table(paste0(PATH, "Validation_Files/pik3ca_stringdb_neighbors.txt"), 
                                     header = T, check.names = F)[,'SYMBOL']
kras_string_neighbors <- read.table(paste0(PATH, "Validation_Files/kras_stringdb_neighbors.txt"), 
                                   header = T, check.names = F)[,'SYMBOL']
idh1_string_neighbors <- read.table(paste0(PATH, "Validation_Files/idh1_stringdb_neighbors.txt"), 
                                   header = T, check.names = F)[,'SYMBOL']
  ...
  enrichments <- lapply(drivers, function(d) {
    ...
    if(length(target_set) == 1) {
      e <- compute_statistical_enrichment(pc_allGenes[pc_allGenes$R_i.name == d,], 
                                          ...
                                          qval_thres)
    # One target set per driver (e.g. STRING neighbors)
    } else {
...
string_neighbors <- list("TP53" = tp53_string_neighbors, 
                         "PIK3CA" = pik3ca_string_neighbors,
                         "KRAS" = kras_string_neighbors,
                         "IDH1" = idh1_string_neighbors)
create_enrichment_barplot(pc_allGenes, drivers, "K-S", string_neighbors, 
                          "STRING", 0.01)
...
############################################################
### SUPPLEMENTAL TABLE 4: TP53 ENRICHMENTS IN OTHER EXTERNAL DATASETS
############################################################ 
### TRRUST ###
