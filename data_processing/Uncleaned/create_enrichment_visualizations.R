...
# for string hg enrichment (q<0.001)
input <- melt(as.data.frame(list("TP53" = -log10(1.571079e-06), "PIK3CA" = -log10(0.00031161), 
                                 "KRAS" = -log10(0.009104977), "IDH1" = -log10(0.7308022))))
# for string hg enrichment (q<0.01, Top 300)
input <- melt(as.data.frame(list("TP53" = -log10(1.36e-06), "PIK3CA" = -log10(2.12e-02), 
                                 "KRAS" = -log10(4.22e-01), "IDH1" = -log10(5.80e-01))))
# for string hg enrichment (q<0.01, Top 500)
input <- melt(as.data.frame(list("TP53" = -log10(2.71e-08), "PIK3CA" = -log10(6.35e-02), 
                                 "KRAS" = -log10(6.45e-01), "IDH1" = -log10(3.38e-02))))
# for string ks enrichment (Top 500)
input <- melt(as.data.frame(list("TP53" = -log10(2.91e-07), "PIK3CA" = -log10(2.45e-01), 
                                 "KRAS" = -log10(3.01e-01), "IDH1" = -log10(3.91e-03))))
# for string ks enrichment (neighbors from STRINGdb)
input <- melt(as.data.frame(list("TP53" = -log10(2.453061e-09), "PIK3CA" = -log10(0.001908791), 
                                 "KRAS" = -log10(0.03330736), "IDH1" = -log10(0.03226985))))
# for string ks enrichment (neighbors from STRINGdb), silent mutations
input <- melt(as.data.frame(list("TP53" = -log10(1.40e-01), "PIK3CA" = -log10(8.55e-01), 
                                 "KRAS" = -log10(1.36e-01), "IDH1" = -log10(3.12e-01))))
# for string ks enrichment (neighbors from STRINGdb), enrichment score
input <- melt(as.data.frame(list("TP53" = 0.313, "PIK3CA" = 0.237, 
                                 "KRAS" = 0.256, "IDH1" = 0.246)))
# for string ks enrichment (neighbors from STRINGdb, naive DE)
input <- melt(as.data.frame(list("TP53" = -log10(4.044087e-06), "PIK3CA" = -log10(0.009604567), 
                                 "KRAS" = -log10(7.160477e-05), "IDH1" = -log10(0.9628466))))
# for string ks enrichment (neighbors from STRINGdb, naive DE), silent
input <- melt(as.data.frame(list("TP53" = -log10(2.42e-04), "PIK3CA" = -log10(2.07e-01), 
                                 "KRAS" = -log10(1.19e-01), "IDH1" = -log10(3.13e-01))))
# for depmap co-dependency enrichment 
input <- melt(as.data.frame(list("TP53" = -log10(0.0220838), "PIK3CA" = -log10(0.6499963), 
                                 "KRAS" = -log10(0.8060139), "IDH1" = -log10(0.5922695))))
...
# For cgc k-s enrichment
input <- melt(as.data.frame(list("TP53" = -log10(9.42e-05), "PIK3CA" = -log10(3.57e-04), 
                                 "KRAS" = -log10(7.21e-02), "IDH1" = -log10(6.01e-06))))
# For cgc k-s enrichment, silent
input <- melt(as.data.frame(list("TP53" = -log10(0.984), "PIK3CA" = -log10(0.997), 
                                 "KRAS" = -log10(0.739), "IDH1" = -log10(0.429))))
# for cgc k-s enrichment, enrichment score
input <- melt(as.data.frame(list("TP53" = -0.272, "PIK3CA" = 0.335, 
                                 "KRAS" = 0.250, "IDH1" = 0.350)))
# For cgc + vogelstein enrichment
input <- melt(as.data.frame(list("TP53" = -log10(2.91e-05), "PIK3CA" = -log10(6.29e-04), 
                                 "KRAS" = -log10(7.03e-01), "IDH1" = -log10(2.24e-06))))
# For naive DE analysis K-S enrichment
input <- melt(as.data.frame(list("TP53" = -log10(0.2486668), "PIK3CA" = -log10(0.02814528), 
                                 "KRAS" = -log10(0.527194), "IDH1" = -log10(0.798624))))
# For naive DE analysis K-S enrichment, silent
input <- melt(as.data.frame(list("TP53" = -log10(0.553), "PIK3CA" = -log10(0.791), 
                                 "KRAS" = -log10(0.581), "IDH1" = -log10(0.614))))
# For dyscovr / naive de analysis cgc k-s statistics
input <- melt(as.data.frame(list("TP53" = (0.085/0.33)*100, "PIK3CA" = (0.078/0.052)*100, 
                                 "KRAS" = (0.016/0.022)*100, "IDH1" = (0.096/0.013)*100)))
# For dyscovr / naive de analysis string k-s statistics
input <- melt(as.data.frame(list("TP53" = (0.050/0.039)*100, "PIK3CA" = (0.039/0.033)*100, 
                                 "KRAS" = (0.024/0.039)*100, "IDH1" = (0.037/0.004)*100)))
colnames(input) <- c("Driver", "EnrichmentScore")


# Calculate an enrichment score according to the protocol given in: https://www.pathwaycommons.org/guide/primers/data_analysis/gsea/
# Calculate a running sum by starting at the top of the ranked list and considering
# each gene in succession: add to the sum if the gene is present in the set, subtract
# if the gene is not in the set. The enrichment score (S) is the maximum value of the sum
# at any point in the list. Do this using the flexgsea package: https://rdrr.io/github/NKI-CCB/ggsea/man/flexgsea.html
pc_allGenes_tp53 <- pc_allGenes[pc_allGenes$R_i.name == "TP53",]
pc_allGenes_tp53$negLogQval <- unlist(lapply(1:nrow(pc_allGenes_tp53), function(i) {
  beta <- pc_allGenes_tp53[i, 'estimate']
  qval <- pc_allGenes_tp53[i, 'q.value']
  return(-log10(qval)* ifelse(beta>0, 1, -1))
}))

pc_naive_df_tp53_sub$negLogQval <- unlist(lapply(1:nrow(pc_naive_df_tp53_sub), function(i) {
  #beta <- pc_allGenes_tp53[i, 'estimate']
  qval <- pc_naive_df_tp53_sub[i, 'Qvalue']
  return(-log10(qval)) #* ifelse(beta>0, 1, -1))
}))

mapping <- as.data.frame(bitr(unique(c(pc_allGenes_tp53$T_k.name, 
                                       pc_naive_df_tp53_sub$gene.name)), 
                              fromType = "SYMBOL", toType = "ENTREZID",
                              OrgDb="org.Hs.eg.db", drop = FALSE))
colnames(mapping) <- c("T_k.name", "T_k.entrez")

pc_allGenes_tp53 <- pc_allGenes_tp53[order(-pc_allGenes_tp53$negLogQval),]
tp53_genelist_dyscovr <- pc_allGenes_tp53$negLogQval
names(tp53_genelist_dyscovr) <- unlist(lapply(pc_allGenes_tp53$T_k.name, function(x) {
  v <- unique(mapping[mapping$T_k.name == x, 'T_k.entrez'])
  if(length(v) > 1) {return(v[1])}
  else {return(v)}
}))

pc_naive_df_tp53_sub <- pc_naive_df_tp53_sub[order(pc_naive_df_tp53_sub$negLogQval),]
tp53_genelist_de <- pc_naive_df_tp53_sub$negLogQval
names(tp53_genelist_de) <- unlist(lapply(pc_naive_df_tp53_sub$gene.name, function(x) {
  v <- unique(mapping[mapping$T_k.name == x, 'T_k.entrez'])
  if(length(v) > 1) {return(v[1])}
  else {return(v)}
}))

term_to_gene <- mapping
term_to_gene$gs_name <- unlist(lapply(mapping$T_k.name, function(t) 
  ifelse(t %fin% tp53_neighbors, "STRING_Target", "Not_STRING_Target")))
term_to_gene$gs_name <- unlist(lapply(mapping$T_k.name, function(t) 
  ifelse(t %fin% cgc_genes, "CGC_Gene", "Not_CGC_Gene")))
term_to_gene <- term_to_gene[,2:3]
colnames(term_to_gene)[1] <- 'entrez_gene'
term_to_gene <- term_to_gene[,c(2,1)]

res_dyscovr <- GSEA(tp53_genelist_dyscovr, pAdjustMethod = 'fdr', 
                    maxGSSize = 20000, TERM2GENE = term_to_gene,
                    pvalueCutoff = 1)

res_de <- GSEA(tp53_genelist_de, pAdjustMethod = 'fdr', 
                    maxGSSize = 20000, TERM2GENE = term_to_gene,
                    pvalueCutoff = 1)
...
generate_empirical_p_string <- function(string_nw, goi, goi_name, master_df, 
  ...
  #string_nw_goi <- string_nw[((string_nw$node1_name == goi_name) | 
    #(string_nw$node2_name == goi_name)),]  # will already be subsetted by our max # of interactions
  string_nw_goi <- string_nw
  print(head(string_nw_goi))
  ...
  if(num_sig_hits > 0) {
    ...
    #hist(distr_vals, main = "", col = "#FFDC91FF",
    #     xlab = paste("\nRandomized STRING Conf. Scores for", 
    #                  paste0(goi_name, paste0("\nSamp. Size = ", 
    #                                          paste0(num_sig_hits,  
    #                                                 paste0(" (q<", paste0(qval_thres, ")")))))),
    #     ylab = paste("Frequency Across", paste(num_trials, "Trials"))) #xlim = c(min(distr_vals), max(distr_vals)))
    
    # Now, get the confidences for each of our actual hits
    real_conf_scores <- unlist(lapply(master_df_goi_sub$T_k.name, function(x) {
      print(x)
      if(x %fin% unique(c(string_nw_goi$node1_name, string_nw_goi$node2_name))) {
        return(as.numeric(unlist(string_nw_goi[(string_nw_goi$node1_name == x) | 
                                                 (string_nw_goi$node2_name == x), 
                             'combined_score'])))
      } else {
        # try using the ENSP ID
        #x_ensp <- all_genes_id_conv[all_genes_id_conv$external_gene_name == x, 'ensembl_peptide_id']
        return(NA)
      }
    }))
    ...
    #abline(v = real_conf_scores_mean, col = "#BC3C29FF", size = 2)
    p <- ggplot(data.frame("distr_vals" = distr_vals), aes(x = distr_vals)) +
      geom_histogram(color = "black", fill = "#FFDC91FF") + theme_minimal() +
      xlab(paste("\nRandomized STRING Conf. Scores for", 
                 paste0(goi_name, paste0("\nSamp. Size = ", 
                                         paste0(num_sig_hits, 
                                                paste0(" (q<", paste0(qval_thres, ")"))))))) + 
      ylab(paste("Frequency Across", paste(num_trials, "Trials"))) +
      geom_vline(aes(xintercept=real_conf_scores_mean), color="#BC3C29FF", size=1) +
...
process_stringdb_network_for_goi <- function(goi, all_genes_id_conv) {
  ...
  neighbors <- string_db$get_neighbors(gene)
  ...
  ensp <- unique(all_genes_id_conv[all_genes_id_conv$external_gene_name == goi, 
                                   'ensembl_peptide_id'])
  interactions <- interactions[(interactions$from %fin% ensp) | 
                                 (interactions$to %fin% ensp),]
  ...
  interactions$combined_score <- unlist(lapply(interactions$combined_score, function(x)
    x/1000))
  interactions <- interactions[interactions$combined_score > 0.4,]
  
  return(na.omit(distinct(interactions)))
}

# Import the String network file
string_nw <- fread("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/Network_Data/STRING/string.9606.protein.links.v11.5.namesAdded.txt")
#string_nw <- string_nw_full[string_nw_full$combined_score >= 400,]
string_nw$combined_score <- unlist(lapply(string_nw$combined_score, function(x) 
  x/1000))
...
interactions <- process_stringdb_network_for_goi(goi_name, all_genes_id_conv)

# Call function
generate_empirical_p_string(string_nw, goi, goi_name, allgenes_pc,
                           0.2, 1000)
#generate_empirical_p_string(interactions, goi, goi_name, allgenes_topDrivers_0.025_ucsf_noLasso,
                           # 0.2, 1000)


# Just look at the HG of overlap between STRING interactors and model hits
# Download the STRINGdb network
get_neighbor_names <- function(string_db, goi, all_genes_id_conv) {
  ...
  #neighbors_uniprot <- unlist(lapply(neighbors, function(x) {
  #  x <- unlist(strsplit(x, ".", fixed = T))[2]
  #  return(all_genes_id_conv[all_genes_id_conv$ensembl_peptide_id == x, 
  #                           'uniprot_gn_id'])
  #}))
  
  neighbors_gn <- unlist(lapply(neighbors, function(x) {
    x <- unlist(strsplit(x, ".", fixed = T))[2]
    return(all_genes_id_conv[all_genes_id_conv$ensembl_peptide_id == x, 
                             'external_gene_name'])
  }))
  return(unique(neighbors_gn))
}
...
tp53_neighbors <- get_neighbor_names(string_db, "TP53", all_genes_id_conv)

compute_statistical_enrichment(pc_allgenes[pc_allgenes$R_i.name == goi, ], 
                               neighbors_gn, "both", 0.01)
...
kras_string_nw_targs_top500 <- unique(unlist(read.csv(paste0(nw_path, "STRING/kras_string_interactions_top500.csv"),
                                                      header = T, check.names = F)[,1:2]))
kras_string_nw_targs_top500 <- kras_string_nw_targs_top500[kras_string_nw_targs_top500 != "KRAS"]
idh1_string_nw_targs_top500 <- unique(unlist(read.csv(paste0(nw_path, "STRING/idh1_string_interactions_top500.csv"),
                                                      header = T, check.names = F)[,1:2]))
idh1_string_nw_targs_top500 <- idh1_string_nw_targs_top500[idh1_string_nw_targs_top500 != "IDH1"]
...
# Import STRINGdb full set of neighbors
tp53_string_nw_targs <- read.table("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/Network_Data/STRING/tp53_stringdb_neighbors.txt", 
                                   header = T, check.names = F)[,1]
pik3ca_string_nw_targs <- read.table("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/Network_Data/STRING/pik3ca_stringdb_neighbors.txt", 
                                   header = T, check.names = F)[,1]
kras_string_nw_targs <- read.table("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/Network_Data/STRING/kras_stringdb_neighbors.txt", 
                                   header = T, check.names = F)[,1]
idh1_string_nw_targs <- read.table("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/Network_Data/STRING/idh1_stringdb_neighbors.txt", 
                                   header = T, check.names = F)[,1]
...
### DEPMAP CODEPENDENCY ###
tp53_crispr_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/TP53's Top 100 Codependencies for CRISPR (DepMap Public 23Q2+Score Chronos).csv",
                              header = T, check.names = F)$Gene
tp53_rnai_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/TP53's Top 100 Codependencies for RNAi (Achilles+DRIVE+Marcotte DEMETER2).csv",
                            header = T, check.names = F)$Gene
tp53_codep <- unique(c(tp53_crispr_codep, tp53_rnai_codep))

pik3ca_crispr_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/PIK3CA's Top 100 Codependencies for CRISPR (DepMap Public 23Q2+Score Chronos).csv",
                              header = T, check.names = F)$Gene
pik3ca_rnai_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/PIK3CA's Top 100 Codependencies for RNAi (Achilles+DRIVE+Marcotte DEMETER2).csv",
                            header = T, check.names = F)$Gene
pik3ca_codep <- unique(c(pik3ca_crispr_codep, pik3ca_rnai_codep))

kras_crispr_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/KRAS's Top 100 Codependencies for CRISPR (DepMap Public 23Q2+Score Chronos).csv",
                                header = T, check.names = F)$Gene
kras_rnai_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/KRAS's Top 100 Codependencies for RNAi (Achilles+DRIVE+Marcotte DEMETER2).csv",
                              header = T, check.names = F)$Gene
kras_codep <- unique(c(kras_crispr_codep, kras_rnai_codep))

idh1_crispr_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/IDH1's Top 100 Codependencies for CRISPR (DepMap Public 23Q2+Score Chronos).csv",
                              header = T, check.names = F)$Gene
idh1_rnai_codep <- read.csv("C:/Users/sarae/Documents/Mona Lab Work/Main Project Files/Input Data Files/Pan-Cancer/DepMap CCLE/IDH1's Top 100 Codependencies for RNAi (Achilles+DRIVE+Marcotte DEMETER2).csv",
                            header = T, check.names = F)$Gene
idh1_codep <- unique(c(idh1_crispr_codep, idh1_rnai_codep))
