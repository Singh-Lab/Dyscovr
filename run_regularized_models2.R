...
#library(caret, lib.loc = library.path)
...
#' method of p-value generation, or shuffled expression DFs for doing randomization_perSamp or
#' randomization_perTarg
#' @param ensg the ENSG ID of the current target gene, such that we can subset the DF appropriately
#' @param num_randomizations if we are doing randomizations per target or per sample, this provides
...
run_regularization_model <- function(formula, lm_input_table, type, debug, 
                                     ...
                                     shuffled_input_dfs, ensg, num_randomizations,
                                     fixed_lambda_for_rand) {
    ...
    tryCatch({
      ...
      if(signif_eval_type == "selectiveInference") {
        ...
        if(fixed_lambda_for_rand) {
          best_model <- eval_signif_randomization(best_model = best_model, sample_ids = lm_input_table$sample_id, 
                                                  x_data = x_data, x_vars = x_vars, y_data = y_data,
                                                 v.group = v.group, lambdas = optimal_lambda, debug = debug, 
                                                 shuffled_input_dfs = shuffled_input_dfs, ensg = ensg, 
                                                 num_randomizations = num_randomizations, signif_eval_type = signif_eval_type)
        } else {
          # Use only ~half the number of possible lambdas as for the real LASSO, for improved runtime
          lambdas <- sort(lambdas)
          lambdas_sub <- c(lambdas[c(TRUE, FALSE)], 100)
          
          best_model <- eval_signif_randomization(best_model = best_model, sample_ids = lm_input_table$sample_id, 
                                                  x_data = x_data, x_vars = x_vars, y_data = y_data, 
                                                  v.group = v.group, lambdas = lambdas_sub, debug = debug, 
                                                  shuffled_input_dfs = shuffled_input_dfs, ensg = ensg, 
                                                  num_randomizations = num_randomizations, signif_eval_type = signif_eval_type)
        }
...
#' @param lambdas a vector of lambda possibilities for cross-validation; if using a fixed lambda,
#' this is a vector of length one with a fixed lambda value
#' @param debug a TRUE/ FALSE value indicating whether or not we are in debug mode
#' @param shuffled_input_dfs a list of shuffled input DFs for doing the randomization_predictor 
#' method of p-value generation or for the per-sample expression method
...
#' @param signif_eval_type the type of randomization we'll be doing (per_predictor, per_target,
#' or per_sample)
eval_signif_randomization <- function(best_model, sample_ids, x_data, x_vars, y_data, v.group, lambdas, 
                                      debug, shuffled_input_dfs, ensg, num_randomizations, signif_eval_type) {
  ...
  if(signif_eval_type == "randomization_predictors") {
    ...
    random_beta_df <- get_per_samp_randomized_beta_df(x_data, sample_ids, v.group, 
                                                      lambdas, shuffled_input_dfs, ensg,
...
#' @param y_data the y-data matrix
#' @param v.group a vector of groups within the group LASSO framework 
#' @param lambdas a vector of lambda possibilities for cross-validation, or one optimal lambda val
#' @param num_randomizations the number of times we will randomize our y-data 
#' in order to generate our null distributions for each x
...
#' @param num_randomizations the number of times we will randomize our y-data 
#' in order to generate our null distributions for each x
#' @param debug a TRUE/FALSE value indicating if we are in debug mode
get_per_samp_randomized_beta_df <- function(x_data, sample_ids, v.group, lambdas, 
  ...
  output_rows <- lapply(shuffled_input_dfs, function(df) {
    y_data_rand <- t(as.matrix(as.numeric(df[index, which(colnames(df) %fin% sample_ids), with = FALSE])))
