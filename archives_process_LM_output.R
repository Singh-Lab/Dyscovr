...
main_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_files/BRCA/tumor_only/"
#main_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_files/BRCA/tumor_normal_matched/"
#main_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_files/Pan-Cancer/tumor_only/"
#main_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_files/Pan-Cancer/tumor_normal_matched/"

# Path to where output figures should be saved
output_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_visualizations/BRCA/tumor_only/"
#output_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_visualizations/BRCA/tumor_normal_matched/"
#output_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_visualizations/Pan-Cancer/tumor_only/"
#output_path <- "/Genomics/grid/users/scamilli/thesis_work/run-model-R/output_visualizations/Pan-Cancer/tumor_normal_matched/"
...
fwrite(master_df, paste(main_path, "TP53/Non-Tumor-Normal Matched/iprotein_output_results_TP53_TMM_bucketCNA_iprot_iciTotFrac_uncorrected.csv", sep = ""))
#fwrite(master_df, paste(main_path, "FOXA1/Non-Tumor-Normal Matched/iprotein_output_results_FOXA1_TMM_rawCNA_iprot_iciTotFrac_uncorrected.csv", sep = ""))
...
fwrite(master_df_mut, paste(main_path, "TP53/Non-Tumor-Normal Matched/iprotein_output_results_TP53_TMM_bucketCNA_iprot_iciTotFrac_MUT_uncorrected.csv", sep = ""))
fwrite(master_df_cna, paste(main_path, "TP53/Non-Tumor-Normal Matched/iprotein_output_results_TP53_TMM_bucketCNA_iprot_iciTotFrac_CNA_uncorrected.csv", sep = ""))
...
fn <- paste(output_path, "TP53/Non-Tumor-Normal-Matched/Mutation, eQTL/Beta Distribution (I-Protein, TMM, bucketCNA, iciTotFrac).png", 
            sep = "")
png(fn, width = 450, height = "350")
visualize_beta_distrib(master_df_mut)
dev.off()

fn <- paste(output_path, "TP53/Non-Tumor-Normal-Matched/CNA, eQTL/Beta Distribution (I-Protein, TMM, bucketCNA, iciTotFrac).png", 
            sep = "")
png(fn, width = 450, height = "350")
...
# Call this function & save output
fn <- paste(output_path, "TP53/Non-Tumor-Normal-Matched/Mutation, eQTL/P-Value Distribution (I-Protein, TMM, bucketCNA, iciTotFrac).png", 
            sep = "")
png(fn, width = 450, height = "350")
visualize_pval_distrib(master_df_mut)
dev.off()

fn <- paste(output_path, "TP53/Non-Tumor-Normal-Matched/CNA, eQTL/P-Value Distribution (I-Protein, TMM, bucketCNA, iciTotFrac).png", 
            sep = "")
png(fn, width = 450, height = "350")
...
create_heat_map <- function(results_table) {
  ...
  matrix <- data.table(matrix(ncol = length(unique(results_table$R_i)),
                              nrow = length(unique(results_table$T_k))))
  colnames(matrix) <- unique(results_table$R_i)
  rownames(matrix) <- unique(results_table$T_k)
  
  # Fill in this table with t-statistics
  for (i in 1:nrow(results_table)) {
    tstat <- results_table$t.statistic[i]
    regprot <- results_table$R_i[i]
    targ <- results_table$T_k[i]
...
# FOR BRCA
tfcancer_df <- read.csv(paste(tfcancer_path, "BRCA/BRCA_TFcancer.csv", sep = ""),
...
# Run function
fn <- paste(output_path, "TP53/Non-Tumor-Normal-Matched/Mutation, eQTL/Known Cancer Gene Enrichment (I-Protein, TMM, bucketCNA, iciTotFrac).png", 
            sep = "")
png(fn, width = 450, height = "350")
plot_cancer_enrichment(master_df_mut_sig, known_cancer_genes_table, tfcancer_df)
dev.off()

fn <- paste(output_path, "TP53/Non-Tumor-Normal-Matched/CNA, eQTL/Known Cancer Gene Enrichment (I-Protein, TMM, bucketCNA, iciTotFrac).png", 
            sep = "")
png(fn, width = 450, height = "350")
