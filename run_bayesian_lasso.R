...
# This file contains two parts, each with an implementation of a Bayesian group LASSO method. See README.md in 
# above repo for in-depth documentation on these methods. 
...
#' Original source code is very minimal/did not run/was unreadable, heavy modifications made
#' Returns: model list object. Attributes are accessed by model$attr 
#' Attributes are:
  #' Sparse_Beta: Vector of size p. The final posterior mean estimate for each regressor.
  #' Groups that contain 0 in their 98% confidence interval are set to 0
  #' To get the coefficients for a particular group, use model$Sparse_Beta[,which(groups==k)] to get group k
  #' Confidence: Vector of size # of groups. Contains confidence score for each group P(Beta_k<0). 
  #' Equivalent to finding tail-size from 0
  #' Betas: (niterations-burnin) x p regressosrs matrix. Contains the gibbs sampling estimate 
  #' for each regressor at each iteration after convergence. Each row describes output at an iteration
  #' To get the history of coefficients for group k, use model$Betas[,which(groups==k)] to get group k
  #' Lambda: (niterations-burnin) length vector. Contains the gibbs sampling estimate for lambda at each iteration
  #' Use mean(model$Lambda) to get the exact posterior estimate
  #' Sigma2: (niterations-burnin) length vector. Contains the gibbs sampling estimate for sigma^2 at each iteration
  #' Use mean(model$Sigma2) to get the exact posterior estimate. 
  #' Tau2: (niteations-burnin) x number of groups matrix. Contains the gibbs sampling estimate for Tau2 for each group at each iteration
  #' Tau2 is a less interpretable parameter, it mainly helps with creating an efficient mathematical formulation
  #' Each row describes the Tau2 for each group at a given iteration
...
runBGL <- function(X, Y, groups, niterations=10000, burnin=5000, scale){
  ...
  #See Slide 11 in the othonormal_basis_justification.pdf for brief explanation of why this is needed
  #This is important for allowing us to update each beta parameter one at a time 
  #At each iteration, we will use these transformation to sample the betas, and then transform back to the original space
  ei  <- eigen(t(X) %*% X) 
  ...
  # Define matrices/vectors that will contain the information of each iteration of our gibbs sampler
  # Save original prior terms
  p <- ncol(X)
  ...
  for (M in 1:niterations)  {
    ...
    #scale all the beta's to orthonormal basis, get the new beta in this basis, then send back up
    scale_beta <- Pt %*% beta.g[M,]
    for(g in 1:G){
      Xg           <- Xg_ALL[[g]]#nX[,which(groups==g)] 
      Xg_not       <- Xg_not_ALL[[g]]#nX[,which(groups!=g)] 
      beta_not     <- scale_beta[which(groups!=g)]  #get latest estimates of all the non-beta terms   
      
      #Define covariance matrix for posterior of group g
      dtau.inv     <- diag(1/tau.sqg[g],nrow=length(which(groups==g)),ncol=length(which(groups==g)))
      ...
      repeat{
        gam[g]  <- rinvGauss(1, nu=sqrt(lambda.g.sq*sig.sqg/sum(betag^2)), lambda=lambda.g.sq)
        if (gam[g] > 0) break    	
      }
      tau.sqg[g] <- 1/gam[g]
    ...
    #Calculating the norm of each group, used for updating other parameter posteriors
    beta.norm <- rep(0, G)
    for(i in 1:G){
    ...
    sh.sig      <- (n-1+p)/2
    sc.sig      <- 1/2*t(Y-X%*%beta.g[M+1,])%*%(Y-X%*%beta.g[M+1,])+ 1/2*sum(beta.norm/tau.sqg)
  ...
  params <- list('Betas'=beta.g[(burnin+1):(1+niterations), ], 'Lambda'=lambdag.post[(burnin+1):(1+niterations)], 'Sigma2'=sigsqg.post[(burnin+1):(1+niterations)], 'Tau2'=tausqg.post[(burnin+1):(1+niterations), ])
...
getConfidence <- function(posteriorBeta, groups){
  ...
  for(i in 1:max(groups)){
    ...
    #Use two different cases for multivariate case (size of group>1) or univariate (size of group==1)
    if(length(means)>1){
      ...
      zero_probs[i] <- pmvnorm(lower=lower, upper=upper, mean=means, sigma=covariance,keepAttr=FALSE)
    }
    else{
      variance <- var(posteriorBeta[,which(groups==i)])
      if(means<0){
        zero_probs[i] <- 1-pnorm(0, mean=means, sd=variance^0.5) #want the probability beta>0 if mean<0, so do 1-pnorm
      }
      if(means>0){
...
#' BGLSS paper provided code that runs well-hyperparameter tuning was added by Amey, 
#' some slight modifications to main BGLSS function were made
#' Returns: model list object. Attributes are accessed by model$attr 
#' Attributes are:
  #' pos_median: Vector of size p. The final posterior MEDIAN estimate for each regressor.
  #' Confidence: Vector of size # of groups. Contains confidence score for each 
  #' group P(Beta_k=0). Lower is better
  #' Betas: (niterations-burnin) x p regressosrs matrix. Contains the gibbs sampling estimate 
  #' for each regressor at each iteration after convergence. Each row describes output at an iteration
  #' To get the history of coefficients for group k, use model$Betas[,which(groups==k)] to get group k
  #' Lambda2: Choice of lambda^2 in the model - it is fit via own method so it's not in the gibbs sampler
...
#' @param nfolds how many folds to use for doing the sparsity tuning. 3 is sufficient
  ...
  if(nfolds!=0){
    ...
    for(modeltype in 1:2){
      ...
      for(i in 1:nfolds){
        ...
        gr_cv<-cv.gglasso(trainX, trainY, group=groups, loss=c('ls'), pred.loss='L2', intercept=FALSE, nfolds=5)
        ...
        if(modeltype==1){
          res<-ModBGLSS(trainY, trainX, group_size=group_sizes, niter=floor(niterations/2), burnin=floor(burnin/2))
        }
        else if(modeltype==2){
          res<-ModBGLSS(trainY, trainX, group_size=group_sizes, a=2*sparsity, b=2*sparsity,niter=floor(niterations/2), burnin=floor(burnin/2))
        }
      ...
      gr_cv<-cv.gglasso(X, Y, group=groups, loss=c('ls'), pred.loss='L2', intercept=FALSE, nfolds=5)
      coefs <- coef(gr_cv$gglasso.fit, s = gr_cv$lambda.1se)
      betas <- coefs[2:length(coefs)]
      sparsity <- calculate_sparsity(betas, groups)
      lambda <- gr_cv$lambda.1se
    ...
    if(best_model==1){
      model <- ModBGLSS(Y, X, group_size=group_sizes, niter=niterations, burnin=burnin)
    }
    else if(best_model==2){
      model<-ModBGLSS(Y, X, group_size=group_sizes, niter=niterations, burnin=burnin, a=2*sparsity, b=2*sparsity)
    }
...
ModBGLSS = function(Y, X, niter = 10000, burnin = 5000, group_size, a=1, b=1,num_update = 100, niter.update =100,
                    verbose = FALSE, alpha=1e-1,gamma=1e-1, pi_prior=TRUE, pi=0.5, update_tau=TRUE,option.weight.group=FALSE,option.update="global",lambda2_update=NULL)
{
  ...
  # Authors suggest doing 100 iterations of Gibbs/EM combo to fit lambda2, then leaving lambda2 fixed for rest of model
  ##################################
  if (update_tau==TRUE){
    ...
    fit_for_lambda2 = fitlambdaEM(Y, X, num_update = num_update, niter = niter.update,group_size = group_size,option.update=option.update,option.weight.group=option.weight.group)
  ...
  list(pos_median = pos_median, Betas = coef, Confidence=zeroprobs, Lambda2=lambda2, zeroProb=all_lg)
}

fitlambdaEM <- function (Y, X, num_update = 100, niter = 100, group_size, a = 1, 
                         b = 1, verbose = FALSE, delta = 0.001, alpha = 0.1, gamma = 0.1, 
                         pi_prior = TRUE, pi = 0.5, option.update = "global", option.weight.group = FALSE) 
{
  n = length(Y)
  p = dim(X)[2]
  ngroup = length(group_size)
    ...
    for (iter in 1:niter) {
      ...
      if (option.weight.group == FALSE) {
        for (i in 1:ngroup) {
          if (Z[i] == 0) {
            ...
            tau2[i] = 1/rig(1, mean = sqrt(matlambda2[i] * 
                                             sigma2/sum(beta[[i]]^2)), scale = 1/(matlambda2[i]))
          }
        }
      }
      else {
        for (i in 1:ngroup) {
          if (Z[i] == 0) {
            tau2[i] = rgamma(1, shape = (group_size[i] + 
                                           1)/2, rate = matlambda2[i] * (group_size[i])/2)
          }
          else {
            tau2[i] = 1/rig(1, mean = sqrt(matlambda2[i] * 
      ...
      for (j in 1:ngroup) beta_vec = c(beta_vec, beta[[j]])
      coef[, iter] = beta_vec
      sigma2 = rinvgamma(1, shape = (n - 1)/2 + sum(Z * 
                                                      group_size)/2 + alpha, scale = (YtY - 2 * t(beta_vec) %*% 
                                                                                        XtY + t(beta_vec) %*% XtX %*% beta_vec + s)/2 + 
                           gamma)
      if (pi_prior == TRUE) 
...
calculate_sparsity <- function(betas, group_assignments){
  #given a list of betas and their respective group assignments, determine how many groups are 0.
  groups <- max(group_assignments)
  sparsity <- 0
  for(i in 1:groups){ #the entire group should be pushed to 0, so just look to see if the group contains a 0
...
convert_grouptype <- function(group_sizes){
  #convert a list of group sizes to a list of group assignments by indices (what gglasso uses)
