...
run_linear_model <- function(protein_ids_df, downstream_target_df, patient_df, 
                             ...
                             fixed_lambda_for_rand, covs_to_incl, removeCis, all_genes_id_conv, 
                             incl_nextMutDriver, run_query_genes_jointly, dataset, inclResiduals) {
...
run_linear_model_per_target_gene <- function(downstream_target_df, methylation_df,
                                             ...
                                             fixed_lambda_for_rand, shuffled_input_dfs, 
  ...
  results_df <- foreach(currentRow = list_of_rows, .export = c("methylation_df",
                        "methylation_df_meQTL", "starter_df", "mutation_targ_df",
                        "cna_df", "expression_df", "log_expression", "cna_bucketing", 
                        "meth_bucketing", "analysis_type", "tumNormMatched", "debug", 
                        "dataset", "randomize", "num_PEER", "num_pcs", "covs_to_incl", 
                        "inclResiduals", "removeCis", "model_type", "collinearity_diagn", 
                        "regularization", "signif_eval_type", "num_randomizations", 
                        "fixed_lambda_for_rand", "shuffled_input_dfs", 
                        "run_query_genes_jointly", "regprot"), 
                        .combine = function(x,y) combine_tab(x, y), 
                        .inorder = FALSE) %dopar% {
