############################################################
### Linear Model ARCHIVE: Last Updated 9-21-21 9AM
...
parser$add_argument("--num_PEER", default = 10, type = "integer",
                    help = "A value from 0 to 10 indicating the number of PEER factors we are using as covariates in our model. Default is 10.")
...
if(debug) {
  ...
  if(exists(methylation_df_meQTL)) {
...
run_linear_model <- function(protein_ids_df, downstream_target_df, patient_df, 
  ...
  results_df_list <- mclapply(1:protein_ids_df[, .N], function(i) {
    ...
    if(exists(methylation_df_meQTL)) {methylation_df_starter <- methylation_df_meQTL}
    else {methylation_df_starter <- methylation_df}
    starter_df <- fill_regprot_inputs(patient_df = patient_df, 
                                      ...
                                      methylation_df = methylation_df_starter, 
                                      cna_df = cna_df,
...
fill_regprot_inputs <- function(patient_df, regprot_i_uniprot, regprot_i_ensg, 
                                ...
                                cna_bucketing, meth_bucketing, analysis_type,
                                tumNormMatched, debug) {
...
fill_targ_inputs <- function(starter_df, targ_k, targ_k_ensg, mutation_targ_df,
                             ...
                             meth_bucketing, tumNormMatched, debug) {
...
construct_formula <- function(lm_input_table, analysis_type, num_PEER, num_pcs,
  ...
  # Remove PEER if we are not including it
  ...
  # Remove the PCs if we are not including them
  pcs <- colnames_to_include[grepl("PC", colnames_to_incl)]
  ...
  if(meth_bucketing) {bucketed_vars <- c(bucketed_vars, "MethStat_k")}
...
############################################################
### Linear Model ARCHIVE: Last Updated 9-16-21 9AM
### Written By: Sara Geraghty, July 2020
############################################################
...
parser$add_argument("--expression_df", default = "expression_tmm_CancerOnly_IntersectPatients.csv", 
                    type = "character",
                    help = "The name of the expression data frame input. [default %(default)s]")
parser$add_argument("--methylation_df", default = "methylation_Beta_CancerOnly_IntersectPatients.csv", 
                    type = "character",
                    help = "The name of the methylation data frame input. [default %(default)s]")
parser$add_argument("--mutation_targ_df", default = "mut_count_matrix_missense_CancerOnly_IntersectPatients.csv", 
                    type = "character",
...
run_linear_model <- function(protein_ids_df, downstream_target_df, patient_df, 
                             mutation_df_targ, mutation_df_regprot, methylation_df, 
                             cna_df, expression_df, analysis_type, tumNormMatched, 
                             randomize, cna_bucketing, meth_bucketing, num_PEER,
                             num_pcs, debug) {
  ...
  results_df_list <- mclapply(1:protein_ids_df[, .N], function(i) {
    ...
    starter_df <- fill_regprot_inputs(patient_df, regprot, regprot_ensg, 
                                      mutation_df_regprot, methylation_df, cna_df,
                                      cna_bucketing, meth_bucketing, tumNormMatched,
                                      debug)
    if(debug) {
      print("Starter DF, with Regprot Inputs")
      print(head(starter_df))
    ...
    regprot_i_results_df_list <- lapply(1:downstream_target_df[, .N], function(k) {
      ...
      # Create a full input table to the linear model for this target
      lm_input_table <- fill_targ_inputs(starter_df, targ, targ_ensg, 
                                         mutation_df_targ, methylation_df,
                                         cna_df, expression_df, cna_bucketing, 
                                         meth_bucketing, tumNormMatched, debug)
